"""RSIChaikBot runtime entrypoint."""
...
class RSIChaikBotRuntime:
    ...
    def _process_candle(self, candle: Candle) -> None:
        ...
        if signal.action == "entry":
            intent = self._prepare_entry(signal)
        elif signal.action == "exit" and position is not None:
            intent = self._build_exit_intent(position, signal)

        append_signal(self.config.storage, signal)

        if intent is not None:
            result = self.execution.place_order(intent, signal)
            self._handle_order_result(result)

        save_state(self.config.storage, self.state)

    def _prepare_entry(self, signal: Signal) -> OrderIntent | None:
        """Apply risk sizing before sending an entry order."""

        if signal.entry_price is None or signal.stop_price is None or signal.take_price is None:
            signal.quantity = 0.0
        ...
        signal.take_price = round_price(signal.take_price, self.config.execution.price_step)
        if quantity <= 0:
    ...
    def _build_exit_intent(self, position: PositionState, signal: Signal) -> OrderIntent:
        """Create an exit intent for the currently open position."""

        exit_price = signal.exit_price if signal.exit_price is not None else self.buffer[-1].close
        signal.exit_price = round_price(exit_price, self.config.execution.price_step)
        return OrderIntent(
            action="exit",
...
def main() -> None:
    ...
    runtime = RSIChaikBotRuntime(config, logger)
    runtime.run()
